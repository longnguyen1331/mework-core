...
namespace Contract
{
    public static class HttpTransfer
    {
        ...
        private static async Task<List<HttpObject.APIresult>> returnListAPI(HttpResponseMessage httpResponseMessage)
        {
            ...
            {
                TGen.Add(new HttpObject.APIresult { code = HttpObject.Enums.Httpstatuscode_API.OK, Data = await httpResponseMessage.Content.ReadAsStringAsync() ?? null, Messenger = "Success! " });
            }
            else
            {
                TGen.Add(new HttpObject.APIresult { code = HttpObject.Enums.Httpstatuscode_API.WARN, Data = await httpResponseMessage.Content.ReadAsStringAsync() ?? null, Messenger = "An error occurred!" });
            }
            return TGen;
        ...
        private static async Task<HttpObject.APIresult> returnAPI(HttpResponseMessage httpResponseMessage)
        {
            ...
            if (httpResponseMessage.IsSuccessStatusCode == true)
            {
                TGen = new HttpObject.APIresult { code = HttpObject.Enums.Httpstatuscode_API.OK, Data = await httpResponseMessage.Content.ReadAsStringAsync() ?? null, Messenger = "Success! " };
            }
            else
            {
                TGen = new HttpObject.APIresult { code = HttpObject.Enums.Httpstatuscode_API.WARN, Data = await httpResponseMessage.Content.ReadAsStringAsync() ?? null, Messenger = "An error occurred!" };
            }
            return TGen;
